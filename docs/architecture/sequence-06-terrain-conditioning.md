...
### Atomic increment 6.9 — Windows validation portability

The operator reported `221 passed, 4 errors` with Windows' 32,767-character environment-variable
limit. Two malformed-HGT test cases had binary payloads as autogenerated pytest IDs, producing
11,539,263- and 25,934,458-character node IDs. Pytest exports each ID through
`PYTEST_CURRENT_TEST`; both setup and teardown failed for each case. Explicit short case IDs
preserve the same malformed inputs and rejection assertions without embedding the raster bytes
in the test name.

Validation on 2026-09-06 reproduced all four errors by enforcing the Windows environment-value
limit around pytest's environment updates. After the fix, the full verifier passed Ruff, strict
...
test portability and does not change terrain products or grant a freeze.
